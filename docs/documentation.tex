...
make test      # compila ed esegue la suite di test automatici
\end{lstlisting}
...
\texttt{server/log.c}, \texttt{server/redis\_threadsafe.c} e
\texttt{common/protocol.c}. Il client da
...
\begin{itemize}[topsep=0.3em]
    ...
    \item avvia il thread periodico per l'invio delle mappe globali mascherate
          (\texttt{periodic\_global}). Il timer di sessione viene invece creato
          all'inizio di ogni partita, quando l'owner invia \texttt{START}.
\end{itemize}
...
La sincronizzazione è realizzata con cinque mutex indipendenti:
\begin{itemize}[topsep=0.3em]
    ...
    \item \texttt{session\_mutex} -- protegge lo stato della sessione
          e il timer;
    \item \texttt{redis\_mutex} (nel modulo \texttt{redis\_threadsafe}) --
          serializza l'accesso al contesto Redis condiviso, evitando
          accessi concorrenti da parte dei thread client durante login
          e registrazione.
\end{itemize}
...
usata è \texttt{user:<nickname>} con valore la password. Le password sono
memorizzate in chiaro: questa scelta è stata adottata per semplicità
didattica e per concentrare il progetto sugli aspetti di comunicazione,
concorrenza e gestione della sessione. In un sistema reale sarebbe
necessario usare hashing sicuro delle password.
...
di escape ANSI e caratteri box-drawing UTF-8 (bordi orizzontali e
verticali), evitando dipendenze da ncurses.
...
Redis thread-safe & \texttt{server/redis\_threadsafe.c} & Mutex per contesto Redis condiviso \\
\hline
...
\begin{enumerate}[topsep=0.3em]
    ...
    \item \textbf{Server multithread con mutex granulari}: un thread per
          ...
          lista client, punteggi, stato sessione e contesto Redis.
    ...
    \item \textbf{Test automatici}: suite di test che coprono il parser
